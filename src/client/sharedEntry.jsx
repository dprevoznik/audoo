      ...
      <div class="flex flex-row">
        ...
        <div class="text-xl text-gray-800 bg-green-300 flex flex-col text-left w-full rounded-r-lg overflow-y-auto">
          <div class="bg-blue-400">
            <span class="font-bold text-orange-700 ml-4 inline-flex">{share.date}</span>
            <span class="text-right object-right">{` | From ${share.sharedBy}`}</span>
          </div>
          <p class="font-bold text-gray-800 m-2 ml-4">{share.memory}</p>
        </div>
      </div>
    </div>
  );
